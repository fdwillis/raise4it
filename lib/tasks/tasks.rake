      ...
      if message.direction == 'inbound' && message.to == ENV["TWILIO_NUMBER"]
        ...
        if !User.all.map(&:support_phone).include?(from_number) && from_number != "4143997341"
          ...
          if raiser_name.present?
            twilio_text.account.messages.create({from: "#{ENV['TWILIO_NUMBER']}", to: from_number, body: "We see you haven't completed your donation to #{User.find_by(username: message.split[1].downcase).business_name}.  To finish your donation please respond with: 25 #{User.find_by(username: message.split[1].downcase).username} and follow the link"})
            puts "text sent"
          else
            puts "Nothing"
          end
